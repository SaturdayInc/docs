...
- An **athlete** (any active Saturday subscriber) gets self-tools over the signed-in account's data.
...
Claude auto-discovers Saturday's authorization server (RFC 9728 / RFC 8414 discovery), walks you through **"Sign in with Saturday,"** and shows a branded consent screen. A coach granting access sees an explicit disclosure that Claude will be able to read their athletes' fueling data and manage their alert settings, and will never write to an athlete's training data.
...
When an athlete connects, Claude can read and write that athlete's Saturday data. Athlete tools operate strictly on the signed-in athlete; there is no athlete selector. The full catalog is in [MCP Integration](/guides/mcp-integration#tool-catalog). Highlights:

- `get_athlete`, `update_athlete` read and update the athlete's profile.
- `list_activities`, `get_activity`, `create_activity` manage the athlete's activities.
...
- `build_bottling_plan`, `record_bottling_choice` turn the prescription into a bottle-by-bottle plan, returned as text and structured content, and record the plan they commit to. [Bottling plans](/guides/mcp-integration#bottling-plans) has the arguments and result shape.
...
A Pro Coach or above additionally sees the tools below. Every athlete argument is confined to the coach's roster (a non-roster athlete returns "resource not found"). They cover the same reads and configuration writes as the [Coach REST API](/guides/coach-api); disabling and re-enabling a webhook endpoint is a REST operation.
...
| `list_webhooks` | `active_only?` | list webhook endpoints (no secrets); `active_only` narrows to active ones |
...
`scope` is `overall` (whole roster), `group` (a coach group), or `athlete` (one athlete), and the most specific scope wins. `scope_id` is required for `group` and `athlete`. For `scope: "athlete"`, `scope_id` is checked against the roster the same way an `athlete_id` is.
...
If a coach's tier lapses mid-session, the coach tools disappear on the next request while the athlete self-tools remain, since the coach is still a subscriber. The result is a degrade to the coach's athlete data, with no error and no broken state. Webhook deliveries to an unentitled coach stop.
