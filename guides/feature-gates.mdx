...
Five keys gate REST routes. Each covers the routes listed beside it.
...
Two more keys gate MCP tools rather than REST routes: `coach` (the coach tools, which also require a coach principal on the request) and `bottling` (`build_bottling_plan` and `record_bottling_choice`). Neither has a shipped default, so each is STEALTH until set in configuration.
