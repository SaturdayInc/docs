...
1. **Create** the activity with type, duration, intensity, and conditions
...
| `intensity` | string | No | `easy`, `moderate`, `hard`, `race` |
| `name` | string | No | Human-readable label |
| `scheduled_at` | string | No | ISO 8601 datetime |
| `conditions` | object | No | Environmental conditions |
| `sub_activities` | array | No | For multi-sport activities |
...
```json
{
  ...
  "intensity": "race",
  "sub_activities": [
    { "type": "swimming", "duration_minutes": 25 },
    { "type": "cycling", "duration_minutes": 75 },
    { "type": "running", "duration_minutes": 55 }
  ]
}
```

Saturday calculates per-segment prescriptions — you fuel differently on the bike than on the run.
