...
`POST /v1/webhooks/{webhook_id}/test` delivers synchronously and returns the delivery record, including the status code your endpoint returned. Use it to confirm signature verification works before you depend on live events. The test event has type `webhook.test`, which is not a subscribable event type; your handler should ignore types it does not recognize rather than erroring on them.
