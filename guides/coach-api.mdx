...
<Note>
  The Coach API requires the **Business tier or higher**. A coach whose tier no longer includes API access degrades cleanly to their own athlete data only — coach endpoints return `404` until an eligible tier is restored. There is no error and no separate "downgrade" call.
</Note>
...
Coach API keys are minted in the [coach portal](https://coach.saturday.fit) under **[Settings → API Keys](https://coach.saturday.fit/admin/api-keys)**. Pass the key as a Bearer token:
