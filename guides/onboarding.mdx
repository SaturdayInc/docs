...
- **`profile_complete: true`** means the inputs are complete. Full-tier responses carry exact numbers (`carb_g_per_hr` and friends); teaser responses still carry ranges.
...
The athlete record's `profile_complete` field, the one `?profile_complete=false` filters on and `athlete.profile_completed` fires for, covers the stored profile only: `sex`, `year_of_birth`, `weight_kg`, `sweat_level`, `saltiness`, `satiety_level`, `fitness_level`, `carb_experience`, `usual_carb_consumption`, and an answered concerns question.

<Note>
`missing_fields` names the safety core as `sex`, `age`, `athlete_weight_kg`, `sweat_level`, `saltiness`, `carb_experience`, `usual_carb_consumption`, `intensity_level`, and `thermal_stress_level`, each marked `required: true`. `age` and `athlete_weight_kg` are the calculation-side names for the athlete's `year_of_birth` and `weight_kg`. `satiety_level`, `fitness_level`, `concerns`, `meal_before_min`, and `is_race` come back as `required: false`, and exactness needs all of them.
</Note>
...
An incomplete-profile response for a request with an `athlete_id` can carry `precision.onboarding.url`, a long-lived athlete-scoped link to Saturday's hosted onboarding page, co-branded with your platform. Check that the URL is present before showing a button, email link, or push action. Requests without an athlete reference do not receive a hosted-onboarding URL.
...
- The link stays valid for 90 days, and every later incomplete-profile response carries a fresh one, so athletes can come back and edit their answers.
