...
<Warning>
  Teaser ranges are display buckets rounded outward from the computed value, and the rounding is not clamped to the caps above. A teaser upper bound can therefore read slightly higher than the engine would ever prescribe, for example `1500-2000` mL/hr against a 1801 mL/hr cap. Treat the upper number in a teaser range as the top of a bucket, not as a target, and never render it as a recommended intake.
</Warning>
