...
Most partner integrations start with API keys. Add OAuth2 when you need athletes or coaches to connect their existing Saturday accounts. Coaches automating against their roster use a coach API key, covered in the [Coach API](/guides/coach-api).
...
Coach keys (`cp_*`) are minted in the [coach portal](https://coach.saturday.fit) under **[API Keys](https://coach.saturday.fit/admin/api-keys)**, not via `api@saturday.fit`. They authenticate the coach to the [Coach API](/guides/coach-api) and are confined to that coach's roster and configuration. A coach whose subscription lapses below Pro Coach loses `cp_` key access on the next request.

Coach keys use a separate scope vocabulary, chosen at mint time in the portal: `roster:read`, `roster:write`, `reports:read`, `org:read`, `org:write`, `org:admin`, `webhooks:manage`, `billing:read`. That is a different model from the partner-key scopes below, and the two do not mix. The portal offers each coach only the scopes their role and plan can mint.

```bash
curl -H "Authorization: Bearer $SATURDAY_API_KEY" \
  https://api.saturday.fit/v1/activity-types
```
...
`environment` is optional and defaults to the environment of the host you call: `live` on `api.saturday.fit`, `test` on the sandbox host. Naming the other environment, or any other value, is a `400` with code `invalid_value`, because a key record only ever authenticates on the host that issued it.

<Note>
  Save the key immediately. The full value is returned once at creation. The `id` in that response is what the rotate and revoke endpoints take, so store it with the key.
</Note>
...
The key prefix and the base URL travel together. Each environment keeps separate key records, so a key issued by the sandbox does not authenticate against `api.saturday.fit`, and the reverse.
...
A key used against the wrong environment fails closed as `invalid_api_key`, and the message names the environment the key was issued for. Each environment keeps separate key records, and a record whose environment is not the host's is refused even if it is present.
